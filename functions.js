...
class bord{
	...
	constructor(){
	 ...
	 for(let j=1;j<33;j++){
		 ...
		 else if(j<26) { p[j]=new piece(j-16,7,"white");pressed[j]="undeturmened";}
	...
	//a[j][l]=(l*55)+(j*55)*8;	
		...
		piece_setup(){
			...
			if(p[j+8].y==8||pressed[j+8]!="undeturmened"){
				...
				//lockwhite=true;
				if(key=='q'&&keyIsPressed||pressed[j+8]=="q"){
			pressed[j+8]="q";
			p[j+8].move(p[j+8]);
			p[j+8].queen(p[j+8]);
				}
				else if(key=='k'&&keyIsPressed||pressed[j+8]=="k"){
					pressed[j+8]="k";
			p[j+8].move(p[j+8]);
			p[j+8].knight(p[j+8]);
				}
				else if(key=='r'&&keyIsPressed||pressed[j+8]=="r"){
			p[j+8].move(p[j+8]);
			p[j+8].rook(p[j+8]);
			pressed[j+8]="r";
				}
